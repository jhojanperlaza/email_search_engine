...
func ProfilingTest(name_dir string, curt_path string) int {
	Browser_dirs(name_dir, curt_path)
	Post_zincsearch()
...
//function that writes the data to a file 'test_test_test_bd_mails.ndjson'
func write_file(dict1 []byte, dict2 []byte) {

	if _, err := os.Stat("test_bd_mails.ndjson"); err == nil {
		//File exists
		f, err := os.OpenFile("test_bd_mails.ndjson", os.O_APPEND|os.O_WRONLY, 0660)
		HandleErr(err)
		str := string(dict1)
		...
		//File does not exist
		f, err := os.Create("test_bd_mails.ndjson")
		HandleErr(err)
		str := string(dict1)
		_, err = fmt.Fprint(f, str, "\n")
		HandleErr(err)
...
func Post_zincsearch() {
	file_found, err := ioutil.ReadFile("test_bd_mails.ndjson")
	HandleErr(err)
